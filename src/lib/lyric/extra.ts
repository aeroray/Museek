...
import { parseAbsoluteWordTimedLyric, parseYrc } from "@/lib/lyrics/parser";
import { decodeQrc } from "@/lib/lyrics/qrc";
...
function extractQrcContent(value: string): string {
  const match = /LyricContent="([\s\S]*?)"/.exec(value);
...
function parseQrcPayload(value: string): LyricInfo | null {
  ...
  const parsed = parseAbsoluteWordTimedLyric(content);
  return parsed.lyric.trim() && parsed.lxlyric?.trim() ? parsed : null;
}
...
async function getTxNumericSongId(songmid: string): Promise<string | null> {
  ...
  try {
    ...
    txSongIdCache.delete(songmid);
    return null;
  }
}

async function getTxNativeLyric(song: MusicInfo): Promise<LyricInfo | null> {
  const songId = await getTxNumericSongId(song.meta.songId);
  ...
  const translation = payload.trans
    ? (parseQrcPayload(payload.trans)?.lyric ?? null)
    : null;
  const romanization = payload.roma
    ? (parseQrcPayload(payload.roma)?.lyric ?? null)
    : null;
  return {
    lyric: parsed.lyric,
...
// QQ's QRC endpoint carries validated word timing. The older endpoint remains
// the fallback because QQ may omit QRC for a song or reject the numeric ID.
async function getTxClassicLyric(songmid: string): Promise<LyricInfo | null> {
  ...
  const res = await httpFetch(url, {
    ...
    headers: {
      ...
      "User-Agent": TX_HEADERS["User-Agent"],
    },
  });
  if (!res.ok) return null;
...
export async function getTxLyric(song: MusicInfo): Promise<LyricInfo | null> {
  try {
    if (!song.meta.songId) return null;
    const native = await getTxNativeLyric(song).catch(() => null);
    return native ?? (await getTxClassicLyric(song.meta.songId));
  } catch {
    return null;
  }
}
...
interface WyNativeLyricResponse extends WyLyricResponse {
  code?: number;
  yrc?: { lyric?: string };
  ytlrc?: { lyric?: string };
...
async function getWyNativeLyric(songId: string): Promise<LyricInfo | null> {
  ...
  const translation = data.ytlrc?.lyric
    ...
    : data.tlyric?.lyric;
  const romanization = data.yromalrc?.lyric
    ? parseYrc(data.yromalrc.lyric).lyric
    : data.romalrc?.lyric;
  if (!native?.lyric.trim() || !native.lxlyric?.trim()) {
    return {
      lyric: data.lrc.lyric,
      tlyric: translation || null,
...
export async function getWyLyric(song: MusicInfo): Promise<LyricInfo | null> {
  try {
    ...
    if (!id) return null;
    const native = await getWyNativeLyric(id).catch(() => null);
...
function decodeKrc(content: string): LyricInfo | null {
  ...
  if (transMatch) {
    ...
    try {
      ...
      for (const item of json.content ?? []) {
        ...
        if (item.type === 1)
          tlyricLines = (item.lyricContent ?? []).map((g) => g.join(""));
      }
    } catch {
      tlyricLines = null;
    }
  ...
  // a normal LRC tag while preserving the inline word timing for the parser.
  ...
  for (const line of str.split("\n")) {
    ...
    const words = decodeName(line.replace(/^\[\d+,\d+\]/, ""));
  ...
  while (sum !== 0n) {
      ...
      if (p > 0n) {
        ...
        y = toLong(
          data[idx] -
            (toLong(
              toLong(y ^ sum) +
                toLong(z ^ key[Number(toLong(toLong(3n & p) ^ e))]),
            ) ^
              toLong(
                toLong(toLong(z >> 5n) ^ toLong(y << 2n)) +
                  toLong(toLong(y >> 3n) ^ toLong(z << 4n)),
              )),
        );
        data[idx] = y;
      } else break;
    ...
    y = toLong(
      data[0] -
        toLong(
          toLong(
            toLong(key[Number(toLong(toLong(p & 3n) ^ e))] ^ z0) +
              toLong(y ^ sum),
          ) ^
            toLong(
              toLong(toLong(z0 >> 5n) ^ toLong(y << 2n)) +
                toLong(toLong(y >> 3n) ^ toLong(z0 << 4n)),
            ),
        ),
    );
    data[0] = y;
...
// Convert decrypted MRC word-timed text into LRC and normalize each word's
// absolute timestamp to the relative offset expected by the shared parser.
function mgParseLyric(str: string): string {
  str = str.replace(/\r/g, "");
  const lineTime = /^\s*\[(\d+),\d+\]/;
  const wordTime = /\((\d+),(\d+)(?:,\d+)?\)/g;
  const lrcLines: string[] = [];
  for (const line of str.split("\n")) {
    ...
    const lineStart = parseInt(result[1]);
    let time = lineStart;
    const ms = time % 1000;
    time = Math.floor(time / 1000);
    const m = String(Math.floor(time / 60)).padStart(2, "0");
    const s = String(time % 60).padStart(2, "0");
    const tag = `${m}:${s}.${String(ms).padStart(3, "0")}`;

    const words = line
      .replace(lineTime, "")
      .replace(
        wordTime,
        (_match, wordStart: string, duration: string) =>
          `<${Math.max(0, Number(wordStart) - lineStart)},${Math.max(1, Number(duration))}>`,
      );
    lrcLines.push(`[${tag}]${words}`);
  }
...
async function mgSearchLyricUrls(song: MusicInfo): Promise<MgLyricUrls | null> {
  ...
  try {
    ...
    if (res.ok) {
      const candidate = (await res.json()) as MgSearchResponse;
      if (candidate.code === "000000") data = candidate;
    }
  } catch {
    // Fall through to the legacy search when jadeite is unavailable.
  }
  if (!data) {
    const legacyUrl =
      `https://app.c.nf.migu.cn/MIGUM2.0/v1.0/content/search_all.do?isCopyright=1&isCorrect=1` +
      `&pageNo=1&pageSize=30&searchSwitch=${encodeURIComponent(
        JSON.stringify({
          song: 1,
          album: 0,
          singer: 0,
      ...
      )}&sort=0&text=${encodeURIComponent(keyword)}`;
    const legacyRes = await httpFetch(legacyUrl, {
      method: "GET",
      headers: {
  ...
  const hasLyricUrl = (item: MgSearchSong): boolean =>
    Boolean(item.lrcUrl || item.lyricUrl || item.mrcurl);
  const byCopyright = song.meta.copyrightId
    ? flat.find((s) => s.copyrightId === song.meta.copyrightId)
    : undefined;
  const bySongId = song.meta.songId
    ? flat.find(
        (s) => s.songId === song.meta.songId || s.id === song.meta.songId,
      )
    : undefined;
  const match =
    [byCopyright, bySongId].find((candidate): candidate is MgSearchSong =>
      Boolean(candidate && hasLyricUrl(candidate)),
    ) ?? flat.find(hasLyricUrl);
  if (!match) return null;
  return {
    lrcUrl: match.lrcUrl ?? match.lyricUrl,
    trcUrl: match.trcUrl,
    mrcUrl: match.mrcurl,
  };
}
...
async function mgResourceLyricUrls(
  copyrightId: string,
): Promise<MgLyricUrls | null> {
  const res = await httpFetch(
...
// resourceinfo.do (fallback), preferring the native word-timed mrcUrl and
// falling back to the plain lrcUrl when MRC is unavailable or malformed.
