...
const dict: Record<Lang, Record<string, string>> = {
  zh: {
    ...
    "sync.title": "文件夹同步（端到端加密）",
    "sync.desc": "把配置加密后写入指定文件夹。若该文件夹被网盘客户端（百度网盘 / OneDrive / iCloud 云盘等）同步，即可在其它设备解密恢复。打开软件会自动拉取较新的备份，退出时自动备份（可在下方关闭）。我们和网盘都看不到明文，且无需登录。",
    "sync.noFolder": "未选择同步文件夹",
    "sync.choose": "选择文件夹",
    "sync.passphrasePlaceholder": "加密口令（恢复时需输入相同口令）",
    "sync.backup": "备份到文件夹",
    "sync.restore": "从文件夹恢复",
    "sync.note": "为实现自动同步，口令会保存在本机；上传到网盘的文件始终为密文。请牢记口令——换设备需输入相同口令，丢失将无法解密。备份文件：museek-config.enc.json。",
  ...
  en: {
    ...
    "settings.tab.playback": "Playback & Download",
    ...
    "sync.title": "Folder sync (end-to-end encrypted)",
    "sync.desc": "Writes your config, encrypted, into a folder you choose. If a cloud client (Baidu Netdisk / OneDrive / iCloud Drive, etc.) syncs that folder, you can decrypt and restore it on another device. Museek auto-pulls a newer backup on launch and backs up on quit (toggle below). Neither we nor the cloud can read the plaintext, and no login is required.",
    ...
    "sync.note": "To enable automatic sync, the passphrase is stored on this device; the file uploaded to the cloud is always ciphertext. Remember it — another device needs the same passphrase, and if lost the backup can't be decrypted. Backup file: museek-config.enc.json.",
    ...
    "sync.wrongPass": "Wrong passphrase or corrupted file",
