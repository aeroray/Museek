...
const dict: Record<Lang, Record<string, string>> = {
  zh: {
    ...
    "about.updateDescInstall": "有新版本 v{version} 可用，可在应用内静默安装并自动重启。",
    "about.download": "前往下载",
    ...
    "about.githubDesc": "源码、议题与发布记录",
    "about.websiteTitle": "官方网站",
    "about.websiteDesc": "产品介绍与下载落地页",
    ...
    "download.deleteFilesDesc": "关闭时仅从列表移除任务，磁盘上的歌曲文件保留（默认）。",
    ...
    "sources.title": "音源管理",
    "sources.hint":
      "开启多个音源后，播放取链按列表顺序分批并行探测（每批最多 3 个），最先返回可用链接的音源胜出；歌词与封面仍按列表顺序依次尝试。",
    "sources.urlPlaceholder":
      "粘贴音源链接（.js），每行一个，可一次导入多个；也可点「本地文件」选择本地脚本：\n例：https://ghproxy.net/raw.githubusercontent.com/.../latest.js",
    ...
    "sources.importJoin": "，",
    "sources.invalidUrl": "请输入有效的音源链接（以 http:// 或 https:// 开头）",
    "sources.invalidUrlLines": "以下内容不是有效链接，请修改后重试：{lines}",
    "sources.empty": "未导入音源",
  ...
  en: {
    ...
    "about.updateDescInstall": "Version v{version} is available. Install silently in-app and relaunch.",
    ...
    "about.gzhHint": "Scan to follow for music sources and updates",
    "about.githubTitle": "GitHub",
    "about.githubDesc": "Source, issues, and releases",
    ...
    "download.deleteFilesDesc": "Off (default): remove the task from the list only; keep the audio file on disk.",
    ...
    "sources.hint":
      "With several sources enabled, playback probes them in list order in parallel waves (up to 3 at a time); the first usable URL wins. Lyrics and covers still try sources one-by-one top to bottom.",
    "sources.urlPlaceholder":
      "Paste source links (.js), one per line — or use “Local file” for scripts on disk:\ne.g. https://ghproxy.net/raw.githubusercontent.com/.../latest.js",
    "sources.paste": "Paste from clipboard",
    "sources.clear": "Clear",
    "sources.import": "Import URL",
    ...
    "sources.importJoin": ", ",
    "sources.invalidUrl": "Please enter a valid source URL (must start with http:// or https://)",
    "sources.invalidUrlLines": "These lines are not valid URLs — fix them and try again: {lines}",
