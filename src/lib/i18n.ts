...
const dict: Record<Lang, Record<string, string>> = {
  zh: {
    ...
    "sync.desc": "把配置加密后写入指定文件夹。若该文件夹被网盘客户端（百度网盘 / OneDrive / iCloud 云盘等）同步，即可在其它设备解密恢复。口令仅你本人知道，我们和网盘都看不到明文，且无需登录。",
    "sync.noFolder": "未选择同步文件夹",
    "sync.choose": "选择文件夹",
    "sync.passphrasePlaceholder": "加密口令（恢复时需输入相同口令）",
    "sync.backup": "备份到文件夹",
    "sync.restore": "从文件夹恢复",
    "sync.note": "口令不会被保存；请牢记，丢失将无法解密恢复。备份文件名为 museek-config.enc.json。",
  ...
  en: {
    ...
    "sync.desc": "Writes your config, encrypted, into a folder you choose. If that folder is synced by a cloud client (Baidu Netdisk / OneDrive / iCloud Drive, etc.), you can decrypt and restore it on another device. Only you know the passphrase — neither we nor the cloud can read the plaintext, and no login is required.",
    ...
    "sync.note": "The passphrase is never saved — remember it; if lost, the backup can't be decrypted. Backup file: museek-config.enc.json.",
