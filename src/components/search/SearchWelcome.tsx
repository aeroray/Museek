...
export function WarmWelcome({ refreshKey }: { refreshKey: string }) {
  ...
  useEffect(() => {
    if (
      messageContext.current.period === period &&
      messageContext.current.refreshKey === refreshKey
    )
      return;
  ...
  return (
    <div className="pointer-events-none mx-2 min-w-0 flex-1 flex h-8 items-center justify-center rounded-md px-3">
      <span
        key={messageKey}
        title={t(messageKey)}
        className="min-w-0 truncate text-center text-base font-medium tracking-tight text-foreground/80 animate-in fade-in duration-300 motion-reduce:animate-none"
        aria-live="polite"
      >
        {t(messageKey)}
      </span>
