...
| **Sources**   | Import lx-music-compatible `.js` sources from local files. Enable, disable, reorder, and use multiple sources for playback failover.                                                                                  | 支持从本地文件导入兼容 lx-music 的 `.js` 音源，可启用、停用、排序，并使用多音源失败转移。                                            |
...
| **Library**   | Favorite songs, playlists, and albums. Organize favorites and local tracks with categories. Import local `mp3`, `flac`, `m4a`, `ogg`, `wav`, and `aac` files or folders, plus CUE sheets that map one album file into per-track clips. | 收藏单曲、歌单和专辑；用分类整理收藏与本地音乐；导入 `mp3`、`flac`、`m4a`、`ogg`、`wav`、`aac` 文件或文件夹，也支持用 CUE 表把一张整轨专辑映射为可单独播放的曲目。 |
...
| Step  | English                                                                                                                                                 | 中文                                                                                                       |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| **1** | Open **Settings -> Sources** and import one or more trusted lx-music-compatible `.js` source files. Enable the sources you want to use. | 打开**设置 -> 音源管理**，导入一个或多个可信的兼容 lx-music 音源文件，并启用需要使用的音源。 |
| **2** | Search for a song, or open **Charts**, **Playlists**, **Albums**, or **Local Music**. Import folders, audio files, or a CUE sheet next to its album file. | 搜索歌曲，或打开**排行榜、歌单、专辑、本地音乐**。可导入文件夹、音频文件，或与整轨音频放在一起的 CUE 表。 |
